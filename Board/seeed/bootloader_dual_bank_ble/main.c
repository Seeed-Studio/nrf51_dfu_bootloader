...
int button_detect(void)
{
    ...
    while (1) {
        ...
        if (t > 20000) {        // More than 2 seconds
            return -1;          // long click
        }
...
int main(void)
{
    ...
    if (!dfu_start) {
        ...
        click = button_detect();
#if defined(GROVE_NODE)
        if (click == 2) {     // double click to run bootloader
#else
        if (click != 0) {     // click, long click or double click to run bootloader
#endif
            dfu_start = true;
